 ...
 * OGP Meta Tags Class
 *
 * Handles OGP and Twitter Card meta tags output.
 *
...
class KSOM_OGP_Meta_Tags {
	 ...
	 * @param KSOM_OGP_Image_Handler $image_handler Image handler instance.
	 */
	...
	public function output_ogp_tags() {
		...
		$tags = apply_filters( 'ksom_ogp_tags', $tags, $this->post );

		if ( empty( $tags ) ) {
			return;
		}

		echo "\n<!-- Kashiwazaki SEO OGP Manager v" . esc_attr( KSOM_VERSION ) . " -->\n";

		// Output robots meta tag if enabled.
		...
		echo "<!-- / Kashiwazaki SEO OGP Manager -->\n\n";
	}
	...
	public function output_twitter_card_tags() {
		...
		$tags = apply_filters( 'ksom_twitter_card_tags', $tags, $this->post );
	...
	private function get_site_name() {
		...
		return apply_filters( 'ksom_ogp_site_name', $site_name );
	}
	...
	private function get_locale() {
		...
		return apply_filters( 'ksom_ogp_locale', $locale );
	}
	...
	private function get_og_type() {
		...
		if ( is_singular() ) {
			...
			$custom_type = get_post_meta( get_the_ID(), '_ksom_og_type', true );
		...
		return apply_filters( 'ksom_og_type', $og_type, $this->post );
	}
	...
	private function get_og_title() {
		...
		if ( is_singular() && $this->post ) {
			$custom_title = get_post_meta( $this->post->ID, '_ksom_og_title', true );
			...
			$title = sprintf( __( 'Search Results for: %s', 'kashiwazaki-seo-ogp-manager' ), get_search_query() );
		} else {
			$title = get_bloginfo( 'name' );
		}

		return apply_filters( 'ksom_og_title', $title, $this->post );
	}
	...
	private function get_og_description() {
		...
		// Check for custom post meta.
		if ( is_singular() && $this->post ) {
			$custom_description = get_post_meta( $this->post->ID, '_ksom_og_description', true );
		...
		return apply_filters( 'ksom_og_description', $description, $this->post );
	}
	...
	private function get_og_url() {
		...
		return apply_filters( 'ksom_og_url', esc_url( $url ), $this->post );
	}
	...
	private function get_og_image() {
		...
		// Check for custom post meta.
		if ( is_singular() && $this->post ) {
			$custom_image = get_post_meta( $this->post->ID, '_ksom_og_image', true );
			if ( ! empty( $custom_image ) ) {
				// Get the image from image handler which will properly encode it.
				$image_url = $this->image_handler->get_post_image( $this->post->ID );
			} else {
				// Use image handler to get the best image.
				$image_url = $this->image_handler->get_post_image( $this->post->ID );
			}
		} else {
			// Use default image.
		...
		return apply_filters( 'ksom_og_image', $image_url, $this->post );
	}
	...
	private function get_og_image_dimensions() {
		...
		return apply_filters( 'ksom_og_image_dimensions', $dimensions, $this->post );
	}
	...
	private function get_twitter_image() {
		...
		return apply_filters( 'ksom_twitter_image', $image_url, $this->post );
	}
	...
	private function get_twitter_card_type() {
		...
		// Check for custom post meta.
		if ( is_singular() && $this->post ) {
			$custom_card_type = get_post_meta( $this->post->ID, '_ksom_twitter_card_type', true );
		...
		return apply_filters( 'ksom_twitter_card_type', $card_type, $this->post );
	}
