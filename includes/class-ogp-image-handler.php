...
/**
 * KSOM_OGP_Image_Handler class.
 *
 * Handles image selection with priority and fallback logic.
 *
 * @since 1.0.0
 */
class KSOM_OGP_Image_Handler {
	...
	public function __construct( $options ) {
		...
		$this->min_width          = apply_filters( 'ksom_ogp_min_image_width', $this->min_width );
		$this->min_height         = apply_filters( 'ksom_ogp_min_image_height', $this->min_height );
		$this->recommended_width  = apply_filters( 'ksom_ogp_recommended_image_width', $this->recommended_width );
		$this->recommended_height = apply_filters( 'ksom_ogp_recommended_image_height', $this->recommended_height );
	}
	...
	public function get_post_image( $post_id ) {
		...
		$custom_image = get_post_meta( $post_id, '_ksom_og_image', true );
	...
	private function get_featured_image( $post_id ) {
		...
		return apply_filters( 'ksom_featured_image_url', $image_url, $post_id );
	}
	...
	public function get_default_image() {
		...
		return apply_filters( 'ksom_default_image_url', $default_image );
	}
	...
	private function validate_image_dimensions( $attachment_id ) {
		...
		return apply_filters( 'ksom_validate_image_dimensions', true, $attachment_id, $width, $height );
	}
	...
	private function is_valid_image_url( $url ) {
		...
		return apply_filters( 'ksom_is_valid_image_url', true, $url );
	}
	...
	public static function has_valid_image_extension( $url ) {
		...
		$valid_extensions = apply_filters( 'ksom_valid_image_extensions', array( 'jpg', 'jpeg', 'png', 'gif', 'webp' ) );
	...
	public function get_image_metadata( $attachment_id ) {
		...
		return apply_filters( 'ksom_image_metadata', $metadata, $attachment_id );
	}
	...
	private function encode_image_url( $url ) {
		...
		return apply_filters( 'ksom_encode_image_url', $encoded_url, $url );
	}
}
